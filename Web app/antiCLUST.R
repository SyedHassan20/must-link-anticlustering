...
ui <- fluidPage(
  ...
  tabsetPanel(
               ...
               column(12, align = "center",
                      tags$p(
                        ...
                        tags$a(
                          ...
                          "Anticlust Webapp Repository",
                          target = "_blank"
                        ),
                        tags$br(),  # Another line break for clarity
                        tags$a(
                          href = "https://github.com/m-Py/must-link-anticlustering/blob/main/Web%20app/synthetic_dataset_20250211.csv",
                          "Synthetic Dataset for Demonstration",
                          target = "_blank"
                        ),
                        tags$br(),  # Another line break for clarity
                        tags$a(
                          href = "https://github.com/m-Py/anticlust",
                          "Anticlust Package Repository",
                          target = "_blank")
                      )
               )
             )
    ...
    tabPanel("Upload Dataset",
             ...
             #   1) Summary table if no must-link
             #   2) Summary table if must-link
             #   3) Assigned Batches table (no must-link)
             #   4) Assigned Batches table (must-link)
             # -----------------------------------------------------------------
             conditionalPanel(
               condition = "input.useMustLink_upload == false",
               fluidRow(
                 column(12, h4("Anticlust Summary Table"),
                        withSpinner(uiOutput("summaryTableUpload")))
               ),
               fluidRow(
                 column(12, h4("Assigned Batches Table"),
             ...
             conditionalPanel(
               ...
               fluidRow(
                 column(12, h4("Must-Link Constrained Anticlust Summary Table"),
                        withSpinner(uiOutput("summaryTableUploadML")))
               ),
               fluidRow(
             ...
             br(),
             div(
               style = "margin-top: 20px;",  # Add spacing above the download button
  ...
  # Footer
  # --------------------------------------------------------------------------
  div(class = "footer", p("Anticlust WebApp by UCSF and Heinrich Heine University Düsseldorf"))
)
...
server <- function(input, output, session) {
  ...
  analysisResult <- eventReactive(input$runBtn, {
    ...
    # Check must-link variable vs features overlap if user checked 'Use Must Link'
    if (isTRUE(input$useMustLink)) {
      req(input$must_link_vars)
      overlap <- intersect(input$must_link_vars, input$features)
      if (length(overlap) > 0) {
        validate(
          need(FALSE, paste(
            "The following variable(s) have been selected for both 'Select Features' and 'Select Must-Link Variable(s)'. To use them as must-link variables, please remove them from 'Select Features' before proceeding.",
    ...
    # Perform normal anticlustering (no must-link) first
  ...
  analysisResultUpload <- eventReactive(input$runUploadBtn, {
    ...
    # If must-link is selected, check overlap with chosen features
    if (isTRUE(input$useMustLink_upload)) {
      req(input$must_link_vars_upload)
      overlap_upload <- intersect(input$must_link_vars_upload, input$features_upload)
      if (length(overlap_upload) > 0) {
        validate(
          need(FALSE, paste(
            "The following variable(s) have been selected for both 'Select Features' and 'Select Must-Link Variable(s)'. To use them as must-link variables, please remove them from 'Select Features' before proceeding.",
            paste(overlap_upload, collapse = ", ")
          ))
        )
      }
    }
    ...
    # ------------------------------------------------------------------------
    # Create summary table(s) with tableone, similarly to synthetic data tab
    # ------------------------------------------------------------------------
    catVars <- names(feature_types)[feature_types == "Categorical"]
    
    summary_output_anticlust_upload <- NULL
    summary_output_anticlustML_upload <- NULL
    
    # If no must-link
    if (!isTRUE(input$useMustLink_upload)) {
      tabAnticlust <- CreateTableOne(
        vars = input$features_upload,
        strata = "BatchAnticlust",
        data = dataset, 
        factorVars = catVars
      )
      summary_output_anticlust_upload <- knitr::kable(
        print(tabAnticlust, smd = TRUE), 
        format = "html", 
        table.attr = "class='table table-striped'"
      )
    ...
    if (isTRUE(input$useMustLink_upload)) {
      ...
      summary_output_anticlustML_upload <- knitr::kable(
        print(tabAnticlustML, smd = TRUE), 
        format = "html", 
        table.attr = "class='table table-striped'"
      )
    ...
    # Return the dataset plus the summary tables
    list(
      data = dataset,
      summary_anticlust_upload = summary_output_anticlust_upload,
      summary_anticlustML_upload = summary_output_anticlustML_upload
    )
  })
  ...
  # Summary table for standard anticlustering (no must-link)
  output$summaryTableUpload <- renderUI({
    req(analysisResultUpload())
    HTML(analysisResultUpload()$summary_anticlust_upload)
  })
  
  # Summary table for must-link anticlustering
  output$summaryTableUploadML <- renderUI({
    req(analysisResultUpload())
    HTML(analysisResultUpload()$summary_anticlustML_upload)
  })
  ...
  output$batchTable <- DT::renderDataTable({
    ...
    data_noML <- analysisResultUpload()$data
  ...
  output$batchTableML <- DT::renderDataTable({
    req(analysisResultUpload())
    req(isTRUE(input$useMustLink_upload))
    data_ML <- analysisResultUpload()$data
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(file) {
      final_data <- analysisResultUpload()$data
